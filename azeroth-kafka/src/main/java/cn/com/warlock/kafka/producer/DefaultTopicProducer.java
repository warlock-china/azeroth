...
public class DefaultTopicProducer implements TopicProducer, Closeable {
    ...
    private static final Logger           log           = LoggerFactory
        .getLogger(DefaultTopicProducer.class);
    ...
    public DefaultTopicProducer(KafkaProducer<String, Object> kafkaProducer,
                                boolean defaultAsynSend) {
    ...
    public boolean publish(final String topicName, final DefaultMessage message, boolean asynSend) {
        ...
        if (asynSend) {
            try {
                ...
                log.error("kafka_send_fail,topic=" + topicName + ",messageId=" + message.getMsgId(),
                    e);
                //同步发送直接抛异常
     ...
     */
    private void doAsynSend(final String topicName, final String messageKey,
                            final DefaultMessage message) {
        // 异步发送
        this.kafkaProducer.send(new ProducerRecord<String, Object>(topicName, messageKey,
            message.isSendBodyOnly() ? message.getBody() : message), new Callback() {
                ...
                public void onCompletion(RecordMetadata metadata, Exception ex) {
                    if (ex != null) {
                        ...
                        log.error("kafka_send_fail,topic=" + topicName + ",messageId=" + messageKey,
                            ex);
                    } else {
                        for (ProducerEventHandler handler : eventHanlders) {
                            try {
                        ...
                        if (log.isDebugEnabled()) {
                            log.debug("kafka_send_success,topic=" + topicName + ", messageId="
                                      + messageKey + ", partition=" + metadata.partition()
