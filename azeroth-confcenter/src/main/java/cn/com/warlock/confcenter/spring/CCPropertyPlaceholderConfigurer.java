package cn.com.warlock.confcenter.spring;
...
import cn.com.warlock.confcenter.utils.ConfigZkPathUtils;
import cn.com.warlock.confcenter.utils.HttpUtils;

public class CCPropertyPlaceholderConfigurer extends PropertyPlaceholderConfigurer
                                             implements DisposableBean {

    private final static Logger logger        = LoggerFactory
        .getLogger(CCPropertyPlaceholderConfigurer.class);
    ...
    protected Properties mergeProperties() throws IOException {
        ...
        if (remoteEnabled) {
            ...
            URL resource = Thread.currentThread().getContextClassLoader()
                .getResource("confcenter.properties");
            if (resource == null)
    ...
    private Map<String, Object> loadRemoteProperties(Properties config) throws FileNotFoundException,
                                                                        IOException {
        Map<String, Object> props = new HashMap<>();
    ...
    private List<File> fecthRemoteConfigFile(Properties config) throws FileNotFoundException,
                                                                IOException {
        List<File> files = new ArrayList<>();
        String classRootDir = Thread.currentThread().getContextClassLoader().getResource("")
            .getPath();
        if (!classRootDir.endsWith("/"))
            classRootDir = classRootDir + "/";
        ...
        for (String url : urls) {
            ...
            try {
                ...
                logger.warn("download file[{}] failture from:{},error:{}", fileName, url,
                    e.getMessage());
            }
        }
    ...
    private List<String> parseAllRemoteUrls(Properties config) throws FileNotFoundException,
                                                               IOException {
        ...
        if (org.apache.commons.lang3.StringUtils.isNotBlank(fileNames)) {
            ...
            for (String file : appFiles) {
                result.add(String.format("%s/download?app=%s&env=%s&ver=%s&file=%s", apiBaseUrl,
                    app, env, version, file));
            }
        }
        //全局配置文件
        ...
        if (org.apache.commons.lang3.StringUtils.isNotBlank(fileNames)) {
            String[] appFiles = StringUtils.commaDelimitedListToStringArray(fileNames);
            for (String file : appFiles) {
                result.add(String.format("%s/download?app=global&env=%s&ver=%s&file=%s", apiBaseUrl,
                    env, version, file));
            }
        }
