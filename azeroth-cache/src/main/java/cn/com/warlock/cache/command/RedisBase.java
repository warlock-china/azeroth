...
import cn.com.warlock.cache.CacheExpires;
import cn.com.warlock.common.serializer.SerializeUtils;
import redis.clients.util.SafeEncoder;

/**
 * ClassName: RedisBase <br/>
 * Function: redis base action. <br/>
 * Reason:  <br/>
 * date: Jan 17, 2017 8:10:04 PM <br/>
 *
 * @author warlock
 * @version 
 * @since JDK 1.8
...
public abstract class RedisBase {
    ...
    public boolean setExpireAt(Date expireAt) {
        try {
            if (isCluster(groupName)) {
                return getBinaryJedisClusterCommands(groupName).pexpireAt(key,
                    expireAt.getTime()) == 1;
            }
    ...
    protected <T> T valueDerialize(byte[] bytes) {
        ...
        try {
            ...
            remove();
            logger.warn("get key[{}] from redis is not null,but Deserialize error,message:{}",
                origKey, e);
            return null;
        }
    }
