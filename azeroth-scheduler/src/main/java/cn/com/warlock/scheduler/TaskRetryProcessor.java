...
public class TaskRetryProcessor implements Closeable {
    ...
    private static final Logger                       logger            = LoggerFactory
        .getLogger(TaskRetryProcessor.class);
    ...
    public void submit(final AbstractJob job, final int retries) {
        ...
        logger.info("Add Job[{}-{}] to retry Queue,will be retry {} time", job.group, job.jobName,
            retries);
    ...
    class PriorityTask implements Runnable, Comparable<PriorityTask> {
        ...
        private void retry() {
            ...
            //重新放入任务队列
            taskQueue.add(this);
            logger.debug("re-submit Job[{}-{}] task to queue,next fireTime:{}", job.group,
                job.jobName, nextFireTime);
        }
