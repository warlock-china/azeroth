package cn.com.warlock.filesystem.factory;
...
import cn.com.warlock.filesystem.provider.fdfs.FdfsProvider;
import cn.com.warlock.filesystem.provider.qiniu.QiniuProvider;
                ...
                if (fsProviders.isEmpty()) {
                    ...
                    for (String g : groups) {
                        ...
                        if (!QiniuProvider.NAME.equals(provider)
                            && !FdfsProvider.NAME.equals(provider)) {
                        ...
                        long connectTimeout = Long
                            .parseLong(ResourceUtils.get("fs." + g + ".connectTimeout", "3000"));
                        int maxThreads = Integer
                            .parseInt(ResourceUtils.get("fs." + g + ".maxThreads", "50"));
                        ...
                        if (QiniuProvider.NAME.equals(provider)) {
                            ...
                            Validate.isTrue(
                                servers != null && servers.matches("^.+[:]\\d{1,5}\\s*$"),
                                "[servers] is not valid");
                            String[] serversArray = servers.split(",|;");
                            fsProvider = new FdfsProvider(urlprefix, g, serversArray,
                                connectTimeout, maxThreads);
                        }
                        //
